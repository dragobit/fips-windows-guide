...
1. **`udp:nat` ホールパンチングをまず試す**(追加インフラ不要): 両側 `advertise: true` + `public: false`、片側が `via_nostr: true`。NIP-59 オファー/アンサー交換後に同時 UDP 打ち抜き。片側がフルコーン/ポート制限 NAT なら高確率で成功。**対称 NAT が絡むと失敗しがち**(観測ポートと実マッピングが食い違うため)。
2. **共通の公開ノード経由のトランジット**(ほぼ必ず繋がる): 直接ピアでなくても通信は成立する。双方が外向きに公開ノード(テストメッシュ、あるいは VPS の自前ノード)とピアれば、そのノードが転送する。エンドツーエンド暗号なので中継は中身を読めない。遅延は1ホップ分増えるが最も堅い。
3. **片側を到達可能にする**: ポートフォワード + `public: true`/`external_addr`、TCP 公開エンドポイント、あるいは **Tor オニオンサービス**(ポートフォワード不要で NAT 内からインバウンド到達可。要 tor デーモン、遅延は大きいが確実)。
4. **グローバル IPv6**: ISP が払い出していれば IPv6 アドレスで直接 advert — NAT をそもそも通らない。

実務上は 1 → 失敗なら 2(または 3)の順で考えるのが簡単です。
